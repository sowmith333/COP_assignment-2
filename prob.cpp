...
void project(){
   ...
   for (auto n:proj_pointsXY){
   	cout << n.id << " " << n.get_X() << " " << n.get_Y() << " " << n.get_Z() <<" \n" ;
   	//find the init and fin in isomeric view and draw line between them
   }
      for (auto n:proj_pointsYZ){
   	cout << n.id << " " << n.get_X() << " " << n.get_Y() << " " << n.get_Z() <<" \n" ;
   	//find the init and fin in isomeric view and draw line between them
   }
      for (auto n:proj_pointsZX){
   	cout << n.id << " " << n.get_X() << " " << n.get_Y() << " " << n.get_Z() <<" \n" ;
   	//find the init and fin in isomeric view and draw line between them
   }


     // project these points using QT 
...
void get_3d(){
   ...
   for(int i=0;i<yz_lines.size();i++){
   		class_line new_line(0.0, 0.0);
   			...
   			for(auto n:d_lines){
   				if((n.get_init()==new_line.init && n.get_fin()==new_line.fin) || (n.get_init()==new_line.fin && n.get_fin()==new_line.init)){
   					new_line.id=d_lines.size()+1;
   					d_lines.push_back(new_line);   					
   				}
   			}
   ...
   for (auto n:d_lines){
   	...
   	//find the init and fin in isomeric view and draw line between them
   	//these are the lines in the 3d object (joining these points)
   }
   for(auto n:d_points){
   	//draw these 3d points in the plane of isomeric view
   	//these are the 3d points. To get the 3d object project these points with the normal of the isometric view with the above lines being joined
   	n.print_point();
   }

// Draw these points and lines using QT
